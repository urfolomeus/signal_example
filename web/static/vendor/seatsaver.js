...
Elm.SeatSaver.make = function (_elm) {
   ...
   model) {
      return function () {
         ...
         {case "Mark":
            return function () {
                 var updateSeat = function (s) {
                    return _U.eq(s.seatNo,
                    action._0) ? _U.replace([["occupied"
                                             ,$Basics.not(s.occupied)]],
                    s) : s;
                 };
                 return _U.replace([["seats"
                                    ,A2($List.map,
                                    updateSeat,
                                    model.seats)]],
                 model);
   ...
   seat) {
      ...
      address,
      Mark(seat.seatNo))]),
      _L.fromArray([$Html.text($Basics.toString(seat))]));
   });
   ...
   var newSeat = F2(function (number,
   occupied) {
      return {_: {}
             ,occupied: occupied
             ,seatNo: number};
   ...
   var saveSeat = Elm.Native.Port.make(_elm).inboundSignal("saveSeat",
   "Int",
   function (v) {
      return typeof v === "number" ? v : _U.badPort("a number",
      v);
   });
   var inputs = A2($Signal.map,
   function (seatNo) {
      return Mark(seatNo);
   },
   saveSeat);
   ...
   initialModel,
   allSignals);
   var reserveSeat = Elm.Native.Port.make(_elm).outboundSignal("reserveSeat",
   function (v) {
      return v;
   },
   A2($Signal.map,
